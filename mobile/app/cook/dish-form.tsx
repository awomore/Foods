...
export default function DishFormScreen() {
  ...
  const loadItem = useCallback(async () => {
    ...
    try {
      ...
      if (item.delivery_window_start) {
        ...
        setWindowStart(item.delivery_window_start.slice(11, 16));
      }
      if (item.delivery_window_end) {
        setWindowEnd(item.delivery_window_end.slice(11, 16));
      }
  ...
  // Load existing discounts if editing
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Build delivery window ISO strings (use available_date as date part)
        ...
        >
          ...
          <View style={styles.section}>
            ...
            {sides.map((s, i) => (
              <View key={i} style={styles.sideRow}>
                ...
                <TouchableOpacity onPress={() => removeSide(i)} style={styles.sideRemove}>
                  <Ionicons name="close" size={16} color={Colors.errorFg} />
                </TouchableOpacity>
              </View>
            ))}
            <View style={styles.addSideWrap}>
              ...
              >
                <Text style={[styles.optToggleText, !newSideOpt && { color: Colors.canvas }]}>
...
const styles = StyleSheet.create({
  ...
  modeBtn: { flex: 1, paddingVertical: 10, borderRadius: Radius.md, borderWidth: 0.5, borderColor: Colors.borderWarm, alignItems: 'center', backgroundColor: Colors.bg },
  modeBtnActive: { backgroundColor: Colors.ink, borderColor: Colors.ink },
  modeBtnText: { fontFamily: Fonts.sansMedium, fontSize: 12, color: Colors.bodySoft },
  modeBtnTextActive: { color: Colors.canvas },

  sideRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.bg, borderRadius: Radius.md, padding: 10, borderWidth: 0.5, borderColor: Colors.borderWarm },
  sideName: { fontFamily: Fonts.sansMedium, fontSize: 13, color: Colors.textInk },
  sideMeta: { fontFamily: Fonts.sans, fontSize: 11, color: Colors.bodySoft, marginTop: 2 },
  sideRemove: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.errorBg },
  addSideWrap: { flexDirection: 'row', gap: 6, alignItems: 'center' },
  optToggle: { paddingHorizontal: 10, paddingVertical: 9, borderRadius: Radius.md, borderWidth: 0.5, borderColor: Colors.borderWarm },
  ...
  discTypeBtn: { flex: 1, paddingVertical: 9, borderRadius: Radius.md, borderWidth: 0.5, borderColor: Colors.borderWarm, alignItems: 'center', backgroundColor: Colors.bg },
  discTypeBtnActive: { backgroundColor: Colors.spice, borderColor: Colors.spice },
  discTypeText: { fontFamily: Fonts.sansMedium, fontSize: 11, color: Colors.bodySoft },
  discTypeTextActive: { color: Colors.canvas },
});
