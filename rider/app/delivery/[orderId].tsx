...
import {
  ...
  TextInput, KeyboardAvoidingView, Platform, ScrollView, Linking, Alert,
} from 'react-native';
...
type Stage = 'pickup' | 'otp_collection' | 'en_route' | 'otp_delivery' | 'done';
...
export default function DeliveryFlowScreen() {
  ...
  useEffect(() => {
    ...
    const broadcastGps = async () => {
      ...
      if (stage !== 'en_route' && stage !== 'otp_delivery') return;
      try {
  ...
  const handleDeliveryOtp = async () => {
    ...
    setOtpError('');
    try {
      const { order: updated } = await riderApi.verifyDeliveryOtp(orderId, otpInput, undefined);
      setOrder(updated);
      setOtpInput('');
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e: any) {
  ...
  const handleSkipDelivery = async () => {
    setActioning(true);
    try {
      const { order: updated } = await riderApi.skipDeliveryOtp(orderId, undefined);
      setOrder(updated);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e: any) {
